...
<html>
	...
	<body style="background-image:url('background.jpg');">
		...
		<nav class="navbar navbar-expand-lg navbar-light bg-dark">
		  ...
		  <h2 style="color:white;margin:0 13px 0 0">TO SUPPLIER</h2>
		  <div class="collapse navbar-collapse" id="navbarSupportedContent">
		    <div class="mr-auto nav-item">
			  <button class="btn btn-outline-light" data-toggle="modal" data-target="#exampleModalLong">To_Supplier List</button>
			</div>
	  ...
	  <?php
		...
		if(array_key_exists('addView',$_POST))	
		{
			if($_POST['addView'] == '0')
			{
				$query = "INSERT INTO `supplied`(`S_ID`,`A_ID`,`S_EMAIL`,`S_ADDRESS`,`S_NAME`) VALUES('".mysqli_real_escape_string($link, $_POST['S_ID'])."','".$_SESSION['UserName']."','".mysqli_real_escape_string($link, $_POST['S_EMAIL'])."','".mysqli_real_escape_string($link, $_POST['S_ADDRESS'])."','".mysqli_real_escape_string($link, $_POST['S_NAME'])."')";
				...
				$query = "INSERT INTO `supplied_contact`(`S_ID`,`CONTACT1`,`CONTACT2`) VALUES('".mysqli_real_escape_string($link, $_POST['S_ID'])."','".mysqli_real_escape_string($link, $_POST['CONTACT1'])."','".mysqli_real_escape_string($link, $_POST['CONTACT2'])."')";
		...
		if(array_key_exists('viewSupplied',$_POST))
		{
			$_SESSION['S_ID'] = mysqli_real_escape_string($link, $_POST['S_ID']);
			
			header("Location: viewSupplied.php");
		}
				
		if(array_key_exists('deleteSupplied',$_POST))
		{
			$query = "DELETE FROM `supplied_contact` WHERE `S_ID` = '".mysqli_real_escape_string($link, $_POST['S_ID'])."' ";
			if(!mysqli_query($link,$query))
			{
				$error = "<p>Could not delete Customer - please try again later.</p>";
			}
			
			$query="DELETE FROM `supplied` WHERE `S_ID` = '".mysqli_real_escape_string($link, $_POST['S_ID'])."' AND `A_ID` = '".$_SESSION['UserName']."'";
			if(!mysqli_query($link,$query))
			{
				$error = "<p>Could not delete Customer - please try again later.</p>";
			}
		...
		<div class="modal fade" id="viewCustomerModal" tabindex="-1" role="dialog" aria-labelledby="viewCustomerModalLabel" aria-hidden="true">
		  <div class="modal-dialog" role="document">
			<div class="modal-content">
			  <form method="post">
				  <div class="modal-header">
					<h5 class="modal-title" id="viewCustomerModalLabel">Find To_Supplier</h5>
					<button type="button" class="close" data-dismiss="modal" aria-label="Close">
					  <span aria-hidden="true">&times;</span>
					</button>
				  </div>
				  <div class="modal-body">
						<div class="container">
							<input type="text" name="S_ID" placeholder="Enter Supplied ID" class="form-control">
						</div>
				  </div>
				  <div class="modal-footer">
					<button type="button" class="btn btn-danger" data-dismiss="modal">Close</button>
					<button name="viewSupplied" type="submit" class="btn btn-dark" style="width:25%;">Find</button>
				  </div>
			  </form>
			</div>
		  </div>
		...
		<div class="modal fade" id="deleteCustomerModal" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
		  <div class="modal-dialog" role="document">
			<div class="modal-content">
			  <form method="post">
				  <div class="modal-header">
					<h5 class="modal-title" id="exampleModalLabel">Delete Supplied</h5>
					<button type="button" class="close" data-dismiss="modal" aria-label="Close">
					  <span aria-hidden="true">&times;</span>
					</button>
				  </div>
				  ...
				  <div class="modal-footer">
					<button type="button" class="btn btn-danger" data-dismiss="modal">Close</button>
					<button type="submit" name="deleteSupplied" class="btn btn-dark">Delete</button>
				  </div>
			  </form>
			</div>
		  </div>
			  ...
			  <div class="modal-body">
				<table class="table table-striped">
				  <?php	
					  ...
					  <tbody>";
				    $result = mysqli_query($link, "SELECT `S_ID`,`S_NAME` FROM `supplied` WHERE `A_ID`= '".$_SESSION['UserName']."' ORDER BY S_NAME");
