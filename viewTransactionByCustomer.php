...
<html>
	...
	<body style="background-image:url('background.jpg');">
		...
		<div class="container" style="margin-top:20px;">
			<form method="post">
			  <div class="form-group row">
				<label class="col-sm-2 col-form-label"><b>Enter Customer Id</b></label>
				<input type="text" class="col-sm-4 form-control" placeholder="Enter Customer Id" name="C_ID">
			...
			<table class="table table-striped">
			  <?php	
				...
				if(array_key_exists('searchSubmit',$_POST))
				{
					...
					{
						$query = "select * from transaction where C_ID = '".$_POST['C_ID']."' and A_ID = '".$_SESSION['UserName']."' ";
						$_SESSION['C_ID'] = $_POST['C_ID'];
					}
					else
					{
						$query = "select * from transaction where C_ID = '".$_POST['C_ID']."' and A_ID = '".$_SESSION['UserName']."' AND T_DATE >= '".$_POST['startSearchDate']."' AND T_DATE <= '".$_POST['endSearchDate']."'";
					}
					...
					while($row = mysqli_fetch_array($result))
					{
						...
						if(mysqli_query($link, "select * from customers where C_ID = '".$_POST['C_ID']."' and A_ID = '".$_SESSION['UserName']."' "))
						{
							$Pgold += $row['P_WEIGHT'];
							$Pamount += $row['AMOUNT'];
							if($row['PAID'] == 0)
								$getAmount += $row['AMOUNT'];
							$C= mysqli_fetch_array(mysqli_query($link, "select * from customers where C_ID = '".$_POST['C_ID']."' and A_ID = '".$_SESSION['UserName']."' "));
							$_SESSION['billPending'] = $C['BILL PENDING'];
						}
						$temp = "
						<tr>
						  <td><b>".$row['P_ID']."</b></td>
		...
		<div class="row">
			<div class="col">
				<p><label><b>Profit from Customer: </b></label><button class="btn btn-success" style="margin-left:20px;"><?php echo $Pamount-$getAmount;?></button></p>
			</div>
			<div class="col">
				<p><label><b>Amount receivable From Customer: </b></label><button class="btn btn-success" style="margin-left:20px;"><?php echo$getAmount;?></button></p>
			</div>
			<div class="col">
				<p><label><b>Gold Purchased By Customer: </b></label><button class="btn btn-success" style="margin-left:20px;"><?php echo$Pgold;?></button></p>
			</div>
		</div>
		  ...
		  <?php
			if(isset($_POST['updatePaid']))
			{
				if(mysqli_query($link,"UPDATE `transaction` SET `PAID`=1 WHERE `P_ID` = '".$_POST['updatePID']."' AND `A_ID`='".$_SESSION['UserName']."'"))
				{
					...
					mysqli_query($link,"UPDATE `customers` SET `BILL PENDING`='".$_SESSION['billPending']."' WHERE `C_ID` = '".$_SESSION['C_ID']."' AND `A_ID`='".$_SESSION['UserName']."'");
				}	
			}
		  ?>
