<?php    
	...
	{
		$query = "UPDATE `supplied` SET `S_ADDRESS` = '".$_POST['S_ADDRESS']."' WHERE `S_ID` = '".$_SESSION['S_ID']."'";
		mysqli_query($link,$query);
		$query = "UPDATE `supplied` SET `S_EMAIL` = '".$_POST['S_EMAIL']."' WHERE `S_ID` = '".$_SESSION['S_ID']."'";
		mysqli_query($link,$query);
		$query = "UPDATE `supplied_contact` SET `CONTACT1` = '".$_POST['CONTACT1']."' WHERE `S_ID` = '".$_SESSION['S_ID']."'";
		mysqli_query($link,$query);
		$query = "UPDATE `supplied_contact` SET `CONTACT2` = '".$_POST['CONTACT2']."' WHERE `S_ID` = '".$_SESSION['S_ID']."'";
		mysqli_query($link,$query);
		$query = "UPDATE `supplied` SET `S_NAME` = '".$_POST['S_NAME']."' WHERE `S_ID` = '".$_SESSION['S_ID']."'";
		mysqli_query($link,$query);
	}
	
	$query = "SELECT `supplied`.*,`supplied_contact`.CONTACT1,`supplied_contact`.CONTACT2 FROM `supplied` INNER JOIN `supplied_contact` ON `supplied`.S_ID = `supplied_contact`.S_ID where `A_ID` = '".$_SESSION['UserName']."' AND `supplied_contact`.S_ID = '".$_SESSION['S_ID']."' ";
...
<html>
	...
	<body style="background-image:url('background.jpg');">
		...
		<nav class="navbar navbar-expand-lg navbar-light bg-dark">
		  ...
		  <h2 style="color:white;margin:0 13px 0 0">TO_SUPPLIED DETAIL</h2>
		  ...
		  <div class="collapse navbar-collapse" id="navbarSupportedContent">
			<div class="ml-auto nav-item">
			  <a href ='tosupplier.php'><button class="btn btn-outline-light" type="submit">Back</button></a>
