<?php    
	...
	{
		$query = "UPDATE `salesmen` SET `S_ADDRESS` = '".$_POST['S_ADDRESS']."' WHERE `S_ID` = '".$_SESSION['S_ID']."'";
		mysqli_query($link,$query);
		$query = "UPDATE `salesmen` SET `S_EMAIL` = '".$_POST['S_EMAIL']."' WHERE `S_ID` = '".$_SESSION['S_ID']."'";
		mysqli_query($link,$query);
		$query = "UPDATE `salesmen_contact` SET `CONTACT1` = '".$_POST['CONTACT1']."' WHERE `S_ID` = '".$_SESSION['S_ID']."'";
		mysqli_query($link,$query);
		$query = "UPDATE `salesmen_contact` SET `CONTACT2` = '".$_POST['CONTACT2']."' WHERE `S_ID` = '".$_SESSION['S_ID']."'";
		mysqli_query($link,$query);
		$query = "UPDATE `salesmen` SET `S_NAME` = '".$_POST['S_NAME']."' WHERE `S_ID` = '".$_SESSION['S_ID']."'";
		mysqli_query($link,$query);
	}
	
	$query = "SELECT `salesmen`.*,`salesmen_contact`.CONTACT1,`salesmen_contact`.CONTACT2 FROM `salesmen` INNER JOIN `salesmen_contact` ON `salesmen`.S_ID = `salesmen_contact`.S_ID where `A_ID` = '".$_SESSION['UserName']."' AND `salesmen_contact`.S_ID = '".$_SESSION['S_ID']."' ";
...
<html>
	...
	<body style="background-image:url('background.jpg');">
		...
		<nav class="navbar navbar-expand-lg navbar-light bg-dark">
		  ...
		  <h2 style="color:white;margin:0 13px 0 0">SALESMAN DETAIL</h2>
		  ...
		  <div class="collapse navbar-collapse" id="navbarSupportedContent">
			<div class="ml-auto nav-item">
			  <a href ='salesmen.php'><button class="btn btn-outline-light" type="submit">Back</button></a>
		...
		<div class="container" style="margin-top:3%;">
			...
			<table class="table" style="margin-bottom:1.5%;">
			  <tbody>
				<thead class="thead-dark"">
					<tr>
					  <th id="1"><h5><b>S_Email</b></h5></th>
					  <th id="1"><h5><b><?php echo $row['S_EMAIL'] ?></b></h5></th>
					</tr>
				</thead>
			  </tbody>
			</table>
